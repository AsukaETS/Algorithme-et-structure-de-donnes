...
public class Fenetre extends JFrame implements ActionListener 
{
   ...
   private Boolean trouve = false;     // booléen pour savoir si on a trouvé
...
/**** Méthode de dessin rattachées à la méthode paint()***********************/
   public void dessineNoeud(Arbre A, int x, int y, Graphics g, Color c1, Color c2, Color c3) {
    
      String s;
      if (A.getRacine()!=null) {s = (String) A.getRacine(); }
      else{ s="";}
   	//si le noeud correspond à un noeud contenant une valeur recherchée
      ...
      g.setColor(c2);
      g.fillOval(x+2,y+2,21+fm.stringWidth(s),21);//on dessine un oval jaune pour l'intérieur en fonction de la taille de la valeur
      g.setColor(c1);
...
// GESTION DES ACTIONS SUITE A UN APPUIS SUR BOUTON : cette methode est declenchée si Un bouton quelconque est appuyé
   public void actionPerformed(ActionEvent e)  // on associe l'evenement souris sur bouton avec l'execution d'un sous prg
   {
      ...
      if (arg.equals("Exemple"))
      {
         Arb = new ArbreCons("Durand", 
             new ArbreCons("Dubois", new ArbreVide(), new ArbreVide()),
             new ArbreCons("Fremond", new ArbreCons("Calobin", new ArbreCons("Calamero"), new ArbreVide()),
             new ArbreCons("Gramont", new ArbreVide(), new ArbreCons("Bremaud"))));
         repaint() ;
      }
   //bouton recherche
      if (arg.equals("Rechercher"))
      {
         ...
         {
         			// resultat = ??
            if (Arb.trouver(elementcherche)) 
            {
               afRech.setBackground(Color.green);trouve = true;
               afRech.setText(" L'élément a été trouvé");
            }
            else {
               afRech.setBackground(Color.red);
               afRech.setText( "non trouvé ...");
            }	
         }
      }
   //bouton supprimer
   ...
   // inserer feuille
      ...
      {
         Arb.insererFeuille(txt.getText());
         repaint();
      }
   ...
   // création du tournoi et remplissage 
